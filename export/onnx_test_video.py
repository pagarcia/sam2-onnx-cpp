...
def interactive_select_points(frame_bgr, session_encoder, session_decoder, encoder_input_size):
    """
    1) Encode the given frame.
    2) Let the user interactively specify seed points 
       (L-click=positive, R-click=negative, M-click=reset).
    3) Return the final chosen points, labels, plus the encoded outputs.
    """
    ...
    def run_decoder_inference():
        """
        Re-run the decoder each time a point is added or reset,
...
def onnx_test_video_mkv_full(args):
    """
    Reads a video file with OpenCV, uses all SAM2 ONNX modules to segment the entire video from a single
    interactive user prompt on the first frame. Overlays the predicted mask in semi-transparent green.
    ...
    encoder_input_size = enc_input_shape[2:]
    ...
    while True:
        ...
        highres_mask = mask_for_mem[:, 0:1, ...]
        t3 = time.time()
        memenc_inputs = {
            "mask_for_mem": highres_mask,
            "pix_feat":     image_embeddings
        }
...
def main():
    ...
    # Always use a PyQt5 file dialog to select the video.
